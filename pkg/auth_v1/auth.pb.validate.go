...
func (m *UserUpdate) validate(all bool) error {
	...
	if wrapper := m.GetName(); wrapper != nil {
		...
		if !_UserUpdate_Name_Pattern.MatchString(wrapper.GetValue()) {
			err := UserUpdateValidationError{
				field:  "Name",
...
var _UserUpdate_Name_Pattern = regexp.MustCompile("^[A-Za-z]+( [A-Za-z]+)*$")
...
var _UserInfo_Name_Pattern = regexp.MustCompile("^[A-Za-z]+( [A-Za-z]+)*$")
